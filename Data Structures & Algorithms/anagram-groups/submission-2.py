class Solution:
    def groupAnagrams(self, strs: List[str]) -> List[List[str]]:
        ...
        for i in range(len(strs)):
            freq_arr = [0] * 26
            for j in strs[i]:
                index = ord(j) - ord('a') # ordinal gives the ascii values for the char and I am using lower becuase ti is guranteed only lower case letters are in the each string
